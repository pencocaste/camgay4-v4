...
export default function ModelPageClient({ username, initialModel }: ModelPageClientProps) {
  ...
  const embedCode = isMobile 
    ? `<iframe src="https://www.camgay4.com/in/?tour=9oGW&campaign=lRUVu&track=embedLCG4movil&room=${username}&disable_sound=1&mobileRedirect=auto&embed_video_only=1" style="width:100%; aspect-ratio:16/9;" frameborder="0" class="code" scrolling="no"></iframe>`
    : `<iframe src="https://www.camgay4.com/in/?tour=Limj&campaign=lRUVu&track=embedLCG4pc&signup_notice=1&b=${username}&disable_sound=1&mobileRedirect=never" height="720px" width="1280px" frameborder="0" class="code" scrolling="no"></iframe>`;

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-xl md:text-2xl font-bold mb-4">{username}&apos;s Live Gay Sex Cam</h1>
      ...
      <div className="flex justify-center mb-8">
        <Button 
          ...
          onClick={() => window.open(`https://www.camgay4.com/in/?tour=dT8X&campaign=lRUVu&track=SSSbuttonCG4&room=${username}`, '_blank')}
        >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {model.tags && model.tags.map((tag) => (
                ...
                >
                  <Badge 
                    className="bg-[#FF5100] hover:bg-[#000000] text-white px-3 py-1 text-sm transition-colors"
                  >
                    {tag}
                  </Badge>
                </Link>
      ...
      {/* Related Models */}
      <RelatedModels currentUsername={username} gender="m" />
    </div>
  );
}